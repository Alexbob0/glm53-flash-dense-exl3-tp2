...
# [dense-overlay] mul1 codebook marker (0x83DCD12D as signed int32) — the
...
EXL3_LINEAR_SUFFIXES = ("trellis", "suh", "svh", "mcg", "mul1")
...
def make_linear_exl3(
    ...
    *,
    ...
    [dense-overlay] mcg became optional and mul1 was added: the dense overlay
...
class Exl3Config(QuantizationConfig):
    ...
    unless the pack config carries a ``non_routed_exl3`` block ([dense-overlay])."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # [dense-overlay] Non-routed dense linear config, written by
        # dense_overlay.py: {"layers": {module_prefix: {"bits": K
        # [, "bf16_shards": [...]]}, ...}[, "modules": [...], "bits": K]}.
        ...
        if nr_bits is not None and int(nr_bits) not in (2, 3, 4, 5, 6):
    ...
    def _bf16_shards_for(self, prefix: str) -> list[int]:
        layers = self.non_routed_exl3.get("layers", {})
        if layers and prefix in layers:
            return list(layers[prefix].get("bf16_shards", []))
        return []
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str):
        ...
        if isinstance(layer, LinearBase):
            # [dense-overlay] pack-declared dense EXL3 linears
...
# [dense-overlay] ------------------------------------------------------------
# Registre des couches denses EXL3 + custom op torch : l'appel LinearEXL3 est
# opaque pour dynamo/inductor (pas de graph break par module) et capturable
# en cudagraph — même approche que les ops vLLM. Sorties fp16 de bout en bout.
...
def _dense_exl3_forward_impl(x: torch.Tensor, handle: int) -> torch.Tensor:
    ...
    for i, linear in enumerate(linears):
        if i in bf16_shards:
            ...
            outputs.append(F.linear(x, w_shard).to(torch.float16))
        else:
            outputs.append(linear.forward(x_fp16, {}, out_dtype=torch.float16))
...
def _dense_exl3_forward_fake(x: torch.Tensor, handle: int) -> torch.Tensor:
    ...
    return x.new_empty((*x.shape[:-1], out), dtype=torch.float16)
...
class Exl3LinearMethod(LinearMethodBase):
    ...
    Ported from vcruz305/vllm-exl3 (validated TP=1 there) with two changes for
    this stack: TP comes from ``layer.tp_rank/tp_size`` (so ``disable_tp``
    replicated layers such as DeepSeekV2FusedQkvAProjLinear load unsharded),
    and both mcg and mul1 codebook markers are accepted (turboderp dense
    tensors are mul1). BF16 mixed shards remain refused when the layer is
    actually TP-sharded.
    """
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # the checkpoint expects for them.
        tp_rank = int(getattr(layer, "tp_rank", 0) or 0)
        ...
        # Shards répliqués intra-couche (KDA in_proj_qkvbfg_a : f_a/g_a) — la
        # classe custom les déclare ; pour eux le checkpoint fournit déjà la
        # taille per-rank, donc pas de narrowing TP au chargement.
        replicated_shards = set(getattr(layer, "replicated_shard_ids", ()) or ())
        ...
        total_out_tiles = sum(s // 16 for s in output_partition_sizes)
        ...
        mcg_param = Parameter(
            torch.zeros(n_shards, 1, dtype=torch.int32), requires_grad=False
        )
        ...
        layer._exl3_linear_output_partition_sizes = list(output_partition_sizes)
        layer._exl3_linear_input_size_per_partition = in_per_partition
        layer._exl3_linear_is_row_parallel = is_row_parallel
        layer._exl3_linear_is_qkv = isinstance(layer, QKVParallelLinear)
        layer._exl3_linear_is_merged = isinstance(layer, MergedColumnParallelLinear)
        layer._exl3_linear_bf16_shards = list(bf16_shards)
    ...
    def _make_weight_loader(
        ...
        def weight_loader(
            ...
            # TP effectif par shard : les shards répliqués chargent tel quel.
            eff_rank = 0 if shard_idx in replicated_shards else tp_rank
            eff_tp = 1 if shard_idx in replicated_shards else tp_size

            if suffix == "weight":
                # BF16 staging: keep declared bf16 shards, discard the stale
                # BF16 copy of EXL3-replaced shards.
                if shard_idx not in bf16_shards:
                    return
                expected_out = output_partition_sizes[shard_idx]
                loaded = loaded_weight.detach()
                if loaded.shape[0] != expected_out:
                    # Shard colonne TP-shardé : le checkpoint fournit la
                    # taille pleine, on découpe la tranche de ce rang.
                    if (not is_row_parallel and eff_tp > 1
                ...
                row_start = sum(
                    output_partition_sizes[i] for i in bf16_shards[:bf16_idx]
                )
            ...
            if suffix == "trellis":
                out_tiles_start = sum(
                    s // 16 for s in output_partition_sizes[:shard_idx]
                )
                out_tiles_end = (
                    out_tiles_start + output_partition_sizes[shard_idx] // 16
                )
                ...
                dest = param.data[
                    out_start : out_start + output_partition_sizes[shard_idx]
                ]
            else:
            ...
            if tuple(dest.shape) != tuple(sharded.shape):
                raise RuntimeError(
                    ...
                    f"loaded {tuple(sharded.shape)} (tp {tp_rank}/{tp_size})"
                )
            dest.copy_(sharded)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        for i in range(n_shards):
            if i in bf16_shards:
                continue
            mcg_set = mcg_vals[i].item() != 0
            mul1_set = mul1_vals[i].item() != 0
            if mcg_set == mul1_set:
                raise RuntimeError(
                    f"EXL3 linear shard {i}: exactly one codebook marker must "
                    f"be set (mcg={mcg_vals[i].item()}, mul1={mul1_vals[i].item()})"
                )
            if mcg_set and mcg_vals[i].item() != MCG_MARKER_SIGNED_INT32:
                raise RuntimeError(
                    f"EXL3 linear shard {i}: bad mcg marker {mcg_vals[i].item()}"
                )
            if mul1_set and mul1_vals[i].item() != MUL1_MARKER_SIGNED_INT32:
                raise RuntimeError(
                    f"EXL3 linear shard {i}: bad mul1 marker {mul1_vals[i].item()}"
                )
        ...
        for i in range(n_shards):
            if i in bf16_shards:
                linears.append(None)
                continue
            t0 = sum(s // 16 for s in output_sizes[:i])
            t1 = t0 + output_sizes[i] // 16
            trellis_shard = layer.trellis[:, t0:t1, :].contiguous()
            suh_shard = layer.suh[i].contiguous()
            svh_shard = layer.svh[
                sum(output_sizes[:i]) : sum(output_sizes[: i + 1])
            ].contiguous()
            mcg_shard = layer.mcg[i].contiguous() if mcg_vals[i].item() else None
            mul1_shard = layer.mul1[i].contiguous() if mul1_vals[i].item() else None
            linears.append(
                make_linear_exl3(
                    trellis_shard,
                    suh_shard,
        ...
        bf16_weight = None
        if bf16_shards and layer.weight.data.shape[0] > 0:
            bf16_weight = layer.weight.data
            layer._exl3_bf16_weight = bf16_weight
        for name in ("trellis", "suh", "svh", "mcg", "mul1", "weight"):
            if hasattr(layer, name):
                try:
                    ...
                    pass
        # Enregistrement pour le custom op (index stable par couche).
        ...
        if not self._logged:
            logger.info(
                "[dense-overlay] EXL3 dense linear active (K=%d, %d shards, custom op)",
                self.bits,
                n_shards,
            )
            self._logged = True
    ...
    def apply(self, layer, x: torch.Tensor, bias: torch.Tensor | None = None):
        ...
        orig_shape = x.shape
        x_2d = x.reshape(-1, orig_shape[-1]) if len(orig_shape) > 2 else x
        y = torch.ops.vllm.dense_exl3_forward(x_2d, handle)
